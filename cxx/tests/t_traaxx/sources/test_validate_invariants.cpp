#include <traaxx/validate_invariants.hpp>
#include <gtest/gtest.h>
#include <algorithm>

namespace
{
    bool has_violation(traaxx::ValidationResult<std::uint32_t> const &result, traaxx::InvariantCheck check)
    {
        return std::any_of(result.violations.begin(), result.violations.end(),
            [check](auto const &v) { return v.check == check; });
    }

    traaxx::Tree<std::uint32_t> exampleTree()
    {
...
{
    auto const result = traaxx::validate_invariants(exampleTree());
...
TEST(ValidateInvariants, DetectsOutOfBounds)
{
    ...
    parent[5] = 999;
    tree = traaxx::Tree<std::uint32_t>{ parent, tree.sibling(), tree.data() };
    auto const result = traaxx::validate_invariants(tree);
    EXPECT_FALSE(result.valid);
    EXPECT_TRUE(has_violation(result, traaxx::InvariantCheck::bounds));
}
...
TEST(ValidateInvariants, DetectsParentOrderViolation)
{
    ...
    auto const result = traaxx::validate_invariants(tree);
    EXPECT_FALSE(result.valid);
    EXPECT_TRUE(has_violation(result, traaxx::InvariantCheck::parent_order));
}
...
TEST(ValidateInvariants, DetectsSiblingOrderViolation)
{
    ...
    sibling[1] = 5;
    tree = traaxx::Tree<std::uint32_t>{ tree.parent(), sibling, tree.data() };
    auto const result = traaxx::validate_invariants(tree);
    EXPECT_FALSE(result.valid);
    EXPECT_TRUE(has_violation(result, traaxx::InvariantCheck::sibling_order));
}
...
TEST(ValidateInvariants, DetectsZeroRoots)
{
    ...
    auto const result = traaxx::validate_invariants(tree);
    EXPECT_FALSE(result.valid);
    EXPECT_TRUE(has_violation(result, traaxx::InvariantCheck::single_root));
}
...
TEST(ValidateInvariants, DetectsSiblingSameParentViolation)
{
    ...
    auto const result = traaxx::validate_invariants(tree);
    EXPECT_FALSE(result.valid);
    EXPECT_TRUE(has_violation(result, traaxx::InvariantCheck::sibling_same_parent));
}
...
TEST(ValidateInvariants, DetectsSiblingInjectiveViolation)
{
    ...
    auto const result = traaxx::validate_invariants(tree);
    EXPECT_FALSE(result.valid);
    EXPECT_TRUE(has_violation(result, traaxx::InvariantCheck::sibling_injective));
}
...
TEST(ValidateInvariants, DetectsUnreachableComponent)
{
    ...
    auto data = std::vector<std::uint32_t>(parent.size());
    auto const tree = traaxx::Tree<std::uint32_t>{ std::move(parent), std::move(sibling), std::move(data) };
    auto const result = traaxx::validate_invariants(tree);
    EXPECT_FALSE(result.valid);
    EXPECT_TRUE(has_violation(result, traaxx::InvariantCheck::parent_order));
    EXPECT_TRUE(has_violation(result, traaxx::InvariantCheck::reachable_from_root));
    EXPECT_FALSE(has_violation(result, traaxx::InvariantCheck::propagation_converges));
    for (auto const &violation : result.violations)
    {
        if (violation.check == traaxx::InvariantCheck::reachable_from_root)
        {
...
TEST(ValidateInvariants, BoundsFailureSuppressesStageTwo)
{
    ...
    tree = traaxx::Tree<std::uint32_t>{ parent, sibling, tree.data() };
    auto const result = traaxx::validate_invariants(tree);
    EXPECT_FALSE(result.valid);
    EXPECT_TRUE(has_violation(result, traaxx::InvariantCheck::bounds));
    EXPECT_FALSE(has_violation(result, traaxx::InvariantCheck::sibling_same_parent));
    EXPECT_FALSE(has_violation(result, traaxx::InvariantCheck::reachable_from_root));
    EXPECT_FALSE(has_violation(result, traaxx::InvariantCheck::propagation_converges));
}
