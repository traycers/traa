...
#include <traaxx/lca_batch.hpp>
#include <traaxx/lca_binary_lifting.hpp>
#include <traaxx/lca_naive.hpp>
#include <traaxx/lca_set.hpp>
...
TEST(Lca, NaiveMatchesWorkedTraces)
{
    ...
    EXPECT_EQ(traaxx::lca_naive(tree, 8u, 11u).value(), 4u);
    EXPECT_EQ(traaxx::lca_naive(tree, 8u, 10u).value(), 6u);
    EXPECT_EQ(traaxx::lca_naive(tree, 10u, 12u).value(), 1u);
    EXPECT_EQ(traaxx::lca_naive(tree, 2u, 8u).value(), 2u);
}

TEST(Lca, BinaryLiftingAgreesWithNaive)
{
    auto const tree = exampleTree();
    auto const node_depth = traaxx::depth(tree.parent(), tree.nodes_count()).value();
    auto const d_max = *std::max_element(node_depth.begin(), node_depth.end());
    auto const table = traaxx::AncestorTable<std::uint32_t>(tree.parent(), d_max);
    EXPECT_EQ(traaxx::lca_binary_lifting(tree, table, node_depth, 8u, 11u), 4u);
    EXPECT_EQ(traaxx::lca_binary_lifting(tree, table, node_depth, 8u, 10u), 6u);
    EXPECT_EQ(traaxx::lca_binary_lifting(tree, table, node_depth, 10u, 12u), 1u);
    EXPECT_EQ(traaxx::lca_binary_lifting(tree, table, node_depth, 2u, 8u), 2u);
}
...
TEST(Lca, BatchAgreesWithBinaryLifting)
{
    ...
    auto const result = traaxx::lca_batch(tree, table, node_depth, a, b);
...
TEST(Lca, SetOfThreeAcrossTwoGroups)
{
    ...
    EXPECT_EQ(traaxx::lca_set(tree, s), 1u);
}
...
TEST(Lca, SetOfTwoWithinOneGroup)
{
    ...
    EXPECT_EQ(traaxx::lca_set(tree, s), 4u);
}
...
TEST(Lca, SetSingletonReturnsItself)
{
    ...
    EXPECT_EQ(traaxx::lca_set(tree, s), 8u);
}

TEST(Lca, AncestorPairReturnsAncestor)
{
    auto const tree = exampleTree();
    EXPECT_EQ(traaxx::lca_naive(tree, 2u, 2u).value(), 2u);
}
