#include <traaxx/bfs_reorder.hpp>
...
TEST(BfsReorder, FreeFunctionMatchesWorkedExample)
{
    ...
    auto const result = traaxx::bfs_reorder(tree);
...
TEST(BfsReorder, ParentIsNonDecreasingAfterReorder)
{
    ...
    auto const tree = traaxx::Tree<std::uint32_t>{ example_parent(), example_sibling(),
        std::vector<std::uint32_t>(example_parent().size()) };
    auto const result = traaxx::bfs_reorder(tree);
    ASSERT_TRUE(result.has_value());
...
TEST(BfsReorder, MultipleRootsProducesValidPermutation)
{
    ...
    auto const tree = traaxx::Tree<std::uint32_t>{ parent, sibling, std::vector<std::uint32_t>(4) };
    auto const result = traaxx::bfs_reorder(tree);
    ASSERT_TRUE(result.has_value());
