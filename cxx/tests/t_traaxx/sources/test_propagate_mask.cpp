...
namespace
{
    ...
    traaxx::BitMask seed_at(std::uint32_t n, std::uint32_t bit)
    {
        auto mask = traaxx::BitMask(n);
        mask.set(bit);
        return mask;
    }

    void expect_bits(traaxx::BitMask const &mask, std::vector<std::uint32_t> const &expected)
    {
...
{
    auto const result
        = traaxx::propagate_down_pointer_doubling(example_parent, seed_at(example_n, 2), example_d_max);
    expect_bits(result, { 2, 4, 6, 8, 9, 10, 11 });
}
...
{
    auto const result
        = traaxx::propagate_up_pointer_doubling(example_parent, seed_at(example_n, 10), example_d_max);
    expect_bits(result, { 0, 1, 2, 4, 6, 10 });
}
...
TEST(PropagateMask, AllThreeStrategiesAgreeOnDown)
{
    ...
    auto const doubling
        = traaxx::propagate_down_pointer_doubling(example_parent, seed_at(example_n, 3), example_d_max);
...
TEST(PropagateMask, AllThreeStrategiesAgreeOnUp)
{
    ...
    auto const doubling
        = traaxx::propagate_up_pointer_doubling(example_parent, seed_at(example_n, 11), example_d_max);
    ASSERT_TRUE(iterative.has_value());
    EXPECT_EQ(*iterative, single_pass);
    EXPECT_EQ(*iterative, doubling);
}
