#include <traaxx_simd/dfs_reorder.hpp>
...
TEST(DfsReorder, FreeFunctionMatchesWorkedExample)
{
    ...
    auto const result = traaxx_simd::dfs_reorder(tree);
...
TEST(DfsReorder, SingleRootIsIdentity)
{
    ...
    auto const tree = traaxx_simd::Tree<std::uint32_t>{ parent, sibling, std::vector<std::uint32_t>{ 42 } };
    auto const result = traaxx_simd::dfs_reorder(tree);
    ASSERT_TRUE(result.has_value());
...
TEST(DfsReorder, DoesNotConvergeReturnsConvergenceError)
{
    ...
    auto const tree = traaxx_simd::Tree<std::uint32_t>{ parent, sibling, std::vector<std::uint32_t>{ 0, 1 } };
    auto const result = traaxx_simd::dfs_reorder(tree);
