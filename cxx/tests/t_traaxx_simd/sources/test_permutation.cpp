...
TEST(Permutation, InverseMatchesShuffledExample)
{
    ...
    auto const old2new = traaxx_simd::inverse(new2old);
...
TEST(Permutation, GatherPlainlyPermutesData)
{
    ...
    auto const gathered = traaxx_simd::gather(data, new2old);
...
TEST(Permutation, RemapIndexVectorTranslatesShuffledParentSibling)
{
    ...
    auto const new2old
        = std::vector<std::uint32_t>{ 8, 3, 0, 11, 5, 1, 9, 12, 2, 6, 4, 10, 7 };
    auto const old2new = traaxx_simd::inverse(new2old);
    auto const parent_prime = traaxx_simd::remap_index_vector(parent, new2old, old2new);
    auto const sibling_prime = traaxx_simd::remap_index_vector(sibling, new2old, old2new);
...
TEST(Permutation, IdentityPermutationIsNoOp)
{
    ...
    auto const remapped = traaxx_simd::remap_index_vector(parent, new2old, old2new);
