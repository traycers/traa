...
{
    EXPECT_EQ(traaxx_simd::level_count_for(1u), 1u);
}

TEST(LevelCountFor, DepthZeroTreatedAsOne)
{
    EXPECT_EQ(traaxx_simd::level_count_for(0u), 1u);
}
...
TEST(LevelCountFor, IsCeilLog2PlusOne)
{
    ...
    EXPECT_EQ(traaxx_simd::level_count_for(5u), 4u);
}

TEST(LevelCountFor, ExactPowerOfTwo)
{
    EXPECT_EQ(traaxx_simd::level_count_for(4u), 3u);
    EXPECT_EQ(traaxx_simd::level_count_for(8u), 4u);
}
