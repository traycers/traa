...
namespace traaxx_simd
{
    ...
    BitMask propagate_up_pointer_doubling(const std::vector<IndexT> &parent, BitMask seed, IndexT d_max)
    {
        ...
        {
            // BitMask combine step stays scalar/parallel: `next.atomic_or(anc[i])`
            // is a scatter to a data-dependent index (same reasoning as
            // propagate_up_iterative — no portable SIMD scatter here).
            auto next = current;
            std::for_each(
                std::execution::par,   //
                indices.cbegin(),      //
