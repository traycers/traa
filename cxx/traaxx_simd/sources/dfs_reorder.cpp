...
{
    // Segmented scan along `sibling` (running left-sibling-subtree-size sum) —
    // inherently sequential (each i depends on running[parent[i]] written by an
    // earlier iteration), not data-parallel; matches cxx/traaxx exactly.
