...
То же самое на дереве из [«01. Сквозной пример»](01-example-tree.md), три снимка (зелёным — узел, отмеченный раньше, жёлтым — только что отмеченный в этом раунде):

```mermaid
flowchart TD
    subgraph S0["исходно: затравка в узле 2"]
        direction TB
        s0((0))
        s1(1)
        s2(2)
    ...
    subgraph S1["после раунда 1, offset=1 (A1=parent)"]
        direction TB
        a0((0))
        a1(1)
        a2(2)
    ...
    subgraph S2["после раунда 2, offset=2 (A2) — итог"]
        direction TB
        b0((0))
        b1(1)
        b2(2)
    ...
    classDef old fill:#9f6,stroke:#333,stroke-width:1px;
    classDef new fill:#fe6,stroke:#333,stroke-width:2px;
    ...
    class b2,b4 old
    class b6,b8,b9,b10,b11 new
```

Раунд 1 не даёт ничего сверх обычного шага (`A1` — это и есть `parent`, отмечается ровно прямой ребёнок узла 2). А вот раунд 2 — это и есть эффект удвоения: сразу пять узлов (`6, 8, 9, 10, 11`) отмечаются **за один раунд**, потому что каждый из них читает маску не у прямого родителя, а у предка на расстоянии 2 (`A2`) — то есть «перепрыгивает» через один уровень. Именно этот скачок и превращает `d` раундов в `log₂ d`.
...
!!! note "Почему `d`, а не `n` — дерево ведь тоже массив"
    Число раундов зависит не от числа ячеек в массиве, а от **длины самой длинной цепочки зависимости**, которую нужно полностью впитать удвоением. На дереве такая цепочка для конкретного узла — путь до корня, и его длина равна **глубине узла**, а не позиции этого узла в массиве `parent`/`sibling`: индекс узла — это просто id по порядку создания, он никак не связан со структурным расстоянием до корня (соседние по id узлы могут лежать в разных ветках). Самая длинная цепочка по всему дереву — это его высота `d`, а не число узлов `n`.

    Проверка, которая всё связывает: у «вырожденной цепочки» (`02-path-coordinate-matrix.md` §3.4, дерево, где у каждого узла ровно один ребёнок) `n = d` — дерево структурно и есть один линейный список. Формула `⌈log₂ d⌉` для неё превращается в `⌈log₂ n⌉` — то есть ровно в формулу для плоского массива ниже. Это не совпадение двух разных правил, а одно и то же правило (`раундов = ⌈log₂(длина самой длинной цепочки)⌉`), где для дерева эта цепочка обычно короче `n` благодаря ветвлению, а в вырожденном случае — совпадает с ним.

### Почему это работает: аналогия с Hillis–Steele scan

Механизм удвоения — не специфика деревьев: это тот же приём, что в классическом параллельном prefix scan Hillis–Steele ([«13. Источники»](13-sources.md) §17.3, видео «Hillis Steele vs Blelloch Scan»). Каждый раунд не продвигает свёртку на шаг, а **склеивает два уже готовых соседних окна в окно вдвое длиннее** — поэтому раундов нужно `⌈log₂ N⌉` (та же формула, что и для дерева выше: здесь цепочка зависимости — весь массив, `y[i]` зависит от всех предыдущих элементов, поэтому длина цепочки равна `N` без остатка). В примере ниже `N=8` — точная степень двойки, поэтому округление незаметно (`⌈log₂ 8⌉ = log₂ 8 = 3`); для произвольной длины, скажем `N=7`, раундов всё равно понадобится `⌈log₂ 7⌉ = 3` — раунды целые, последний выполняется полностью, даже если использует не весь свой запас.
