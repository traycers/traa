...
- через `propagate` (итеративная свёртка, возвращают `std::expected<std::vector<IndexT>, ConvergenceError>`): `depth` (по `parent`, `f = v[p]+1`), `rank` (по `sibling`);
- однопроходный топологический scan, не `propagate` (голый `std::vector<IndexT>`, `std::expected` не нужен — сходимости как понятия нет): `size`;
- прямой O(n) проход без итераций (голый `std::vector<...>`, `std::expected` не нужен): `nsibling`, `isleaf` (→ `std::vector<bool>`), `firstchild`, `lastchild`.

**Правка (найдена при подготовке реализации, по аналогии с [[11-lca-variants]]):** `size` изначально был отнесён к «через `propagate` с `f=+`» — это ошибка той же природы, что и исходная классификация LCA-множества в 11. `propagate<...>` (01) — это «повторять `step`, пока `current != next`»; для идемпотентной `f` (`or`, как в маске §5.1/§5.2) повторное применение безвредно и сходится. Для неидемпотентной `f=+` («вниз-вверх» аккумуляция, `size[parent[i]] += size[i]`) наивный повтор того же скаттера на каждой итерации **никогда не сходится** для любого дерева с ветвлением — значение продолжает расти на каждой итерации, `propagate<...>` всегда вернёт `ConvergenceError` вместо корректного ответа. `size`, как и `lca_set` (§10.6, см. правку в [[11-lca-variants]]), — однопроходный топологический scan по образцу §5.3/§8.2 шаг 2 (`size[i]=1` инициализация, затем один проход `i` от `n-1` до `1`, `size[parent[i]] += size[i]`, требует `parent[i] <= i`), не `propagate`-примитив. Сигнатура: `size(const std::vector<IndexT>& parent) -> std::vector<IndexT>` — без `max_iterations`, без `std::expected`.
